...
from plate_detector.layers.functions.prior_box import PriorBox
from plate_detector.utils.nms.py_cpu_nms import py_cpu_nms
import cv2
from plate_detector.models.retina import Retina
from plate_detector.utils.box_utils import decode, decode_landm
import time
import torchvision
import os
...
def detection_plates(picpath, net, devices=device):
    torch.set_grad_enabled(False)
    cfg = cfg_mnet

    # net and model
    # net = Retina(cfg=cfg, phase='test')
    # net = load_model(net, './plate_detector/weights/mnet_plate.pth', False)
    # net.eval()
    ...
    for img_name in contents:
        ...
        if args.save_image:
            for b in dets:
                ...
                # plot box
                # from tongan.utils.general import plot_one_box
                # xyxy = (x1, y1, x2, y2)
                # plot_one_box(xyxy, img,
                #              color=[0, 255, 0], line_thickness=2)
                # cv2.imwrite('imgplate.png', img)
                
                # from recog_plate import get_license
                # txt, color, _, _ = get_license(img)
                # print(txt, color)

                ans.append((x1, y1, w, h))
...
if __name__ == "__main__":
    picname = './samples/testcar.jpg'
    pos = detection_plates(picname)
    print("detected {} plates.".format(len(pos)))
    print(pos)
